...
def is_macro_call(ast, env):
    return (
        ...
        and len(ast) > 0
        and isinstance(ast[0], maltypes.Symbol)
        and ast[0].name in env
        and isinstance(env[ast[0].name], maltypes.MalFunc)
        and env[ast[0].name].is_macro
    )


def macroexpand(ast, env):
    while is_macro_call(ast, env):
        ast = env[ast[0].name](*ast[1:])
    return ast
